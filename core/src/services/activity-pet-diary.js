 ...
 * 萌宠成长日记（S3）活动服务
 *
 * 从上游 core/src/services/activity-center/pet-diary.ts 移植（TS→JS）。
 * 协议以上游为准：命令字与选择器来自官方小程序 1.14.0.1 编码器，不按响应长度推断。
 *
 * 与上游的差异：
 * - 素材前缀改为 /activity/pet-diary/（本地惯例，见 AGENTS.md 活动静态素材约定）
 * - getCurrentSolarTerms 适配本地 getSolarTermsInfo（本地字段为 claimable，id 为数字）
 * - 节令领取复用本地 claimSolarTermsReward，不用上游的类型化 ClaimSolarTermsRequest
 */
...
// 素材路径：协议下发 Cocos 路径 -> 本地文件名（文件名即契约，不得重命名）
const assetPaths = new Map(
    assets.map(entry => [entry.path.replace(/\/spriteFrame$/, ''), `/activity/pet-diary/${entry.file}`]),
);

// 命令字与响应选择器均来自官方编码器，不得按响应长度或 UI 文案推断
const OPERATIONS = {
...
function createPetDiaryService(deps) {
    const {
        ...
        serializeMutation, getCurrentSolarTerms, claimSolarTerm,
    } = deps;

    let pendingRead = null;

    const fail = (message) => { throw businessError('PET_DIARY_UNAVAILABLE', message); };
    const list = (value) => (Array.isArray(value) ? value : []);
    const items = (value) => list(value).map(itemDto);
    const localImage = (value) => assetPaths.get(String(value || '').replace(/\/spriteFrame$/, '')) || '';
    const json = (value) => { try { return JSON.parse(String(value || '{}')); } catch { return {}; } };
    const plain = (type, value) => types.PetDiaryOperateReply.root
        .lookupType(`gamepb.activitypb.${type}`)
        .toObject(value, { longs: String, defaults: true });

    async function rpc(method, type, replyType, input) {
        const request = types[type].fromObject(input);
        const { body } = await sendMsgAsync(
            'gamepb.activitypb.ActivityService',
            method,
            Buffer.from(types[type].encode(request).finish()),
        );
    ...
    async function operate(activityId, command, selector, params = {}) {
        ...
        if (str(reply.activity_id) !== activityId || num(reply.operate_type) !== command) {
    ...
    async function readGroup() {
        // 本地 GetGroupRequest 字段名为 id（上游为 group_id），字段号同为 1，线格式一致
        const reply = await rpc('GetGroup', 'ActivityGetGroupRequest', 'PetDiaryGetGroupReply', { id: GROUP_ID });
        ...
        return {
            pet,
            seeds: children.find(entry => str(entry.head?.id) === SEEDS_ID),
            shop: children.find(entry => str(entry.head?.id) === SHOP_ID),
        };
    }
    ...
    const feedCosts = () => base.feed_items.split(';').map((value) => {
        const [id, count] = value.split(':');
        return { id, count };
    });
    const charmRefreshCost = () => ({
        id: String(refresh.manual_refresh_cost_id),
        count: String(refresh.manual_refresh_cost_count),
    });
    const charmNeedsChoice = (battle) => list(battle.charm_equipped).length > 0 && battle.charm_pick_used !== true;
    ...
    function treasureDto(value) {
        return {
            ...
            protectedCount: str(value.protected_count),
            originalCount: str(value.original_count),
            maxCount: str(value.max_count),
            startTime: num(value.start_at) * 1000,
            endTime: num(value.end_at) * 1000,
            createdTime: num(value.created_at) * 1000,
            sourceCharmIds: list(value.source_charm_ids).map(Number),
            plunderCount: num(value.plunder_count),
            maxPlunderCount: num(value.max_plunder_count),
            previews: list(value.battle_previews).map(p => ({
                challengeId: str(p.challenge_item_id),
                canStart: p.can_start === true,
                maxProfit: itemDto(p.max_profit),
                maxLoss: itemDto(p.max_loss),
                plunderableCount: str(p.plunderable_count),
            })),
        };
    }

    function normalize(group, bag, solar = null, warnings = []) {
        ...
        const charmDto = (id) => {
            ...
            return {
                id,
                name: config?.name || `锦囊 ${id}`,
                description: config?.desc || '',
                shortDescription: config?.short_desc || config?.desc || '',
                useLimit: config?.use_limit ?? 0,
                image: localImage(config?.icon_path),
                remaining: list(battle.charm_effect_remaining_count)
                    .filter(e => num(e.charm_id) === id)
                    .map(e => num(e.remaining_count)),
            };
        };

        const goods = list(group.shop?.shop?.goods).map((g) => {
            const limit = BigInt(str(g.purchase_limit));
            const purchased = BigInt(str(g.purchased_count));
            const remaining = limit > 0n ? (limit > purchased ? limit - purchased : 0n).toString() : null;
            const safeCosts = list(g.cost).length > 0
                && list(g.cost).every(c => str(c.id) !== DIAMOND_ID)
                && num(g.diamond_cost_count) === 0;
            return {
                id: str(g.id),
                name: String(g.name),
                image: localImage(json(g.desc).res) || itemDto(list(g.item)[0]).image,
                rewards: items(g.item),
                costs: items(g.cost),
                limit: str(g.purchase_limit),
                purchased: str(g.purchased_count),
                remaining,
                exchangeable: active && isActive(group.shop?.head) && safeCosts
                    && remaining !== '0' && costsAvailable(list(g.cost), bag),
                safeCosts,
                order: num(g.order),
        ...
        const canChooseCharm = active && adult && battle.charm_pick_used !== true
            && list(battle.charm_daily_pool).length > 0;
        ...
        return {
            ...
            title: '萌宠成长日记',
            active,
            startTime: num(group.pet.head.start_time) * 1000,
            endTime: num(group.pet.head.end_time) * 1000,
            ...
            balances: [1028, 1029, 80101, 80102, 80103, 1002].map(id => ({
                ...itemDto({ id, count: bag?.get(String(id)) || '0' }),
                known: bag !== null,
            })),
            nurture: {
                ...
                feedCount: num(state.feed?.feed_count),
                feedLimit: base.daily_feed_limit,
                feedCosts: items(feedCosts()),
                canFeed: active && !adult && num(nurture.stage) === 1
                    && num(state.feed?.feed_count) < base.daily_feed_limit
                    && costsAvailable(feedCosts(), bag),
            },
            hunt: {
                count: num(hunt.treasure_count),
                limit: base.daily_treasure_limit,
                total: str(hunt.treasure_total),
                luckyStarTotal: str(hunt.lucky_star_gained_total),
                costs: items(hunt.treasure_cost),
                canDraw: active && adult && num(hunt.treasure_count) < base.daily_treasure_limit
                    && costsAvailable(list(hunt.treasure_cost), bag),
                canPlunder: active && hunt.can_play_plunder === true
                    && num(battle.battle_count) < fight.daily_battle_limit,
            },
            seeds: {
                canClaim: active && isActive(group.seeds?.head) && canClaimSeeds,
                days: availableSeeds.map(r => ({
                    day: num(r.unlock_day),
                    claimed: r.claimed === true,
                    claimable: r.claimable === true,
                    rewards: items(r.reward),
                })),
            },
            stories: list(state.story?.stories).map((s) => {
                const desc = json(s.selected_desc);
                return {
                    order: num(s.order),
                    ...
                    captionImage: localImage(desc.say),
                    caption: localImage(desc.say) ? '' : String(desc.say || ''),
                };
            }),
            charms: {
                pool: list(battle.charm_daily_pool).map(charmDto),
                equipped: list(battle.charm_equipped).map(charmDto),
            ...
            treasures: list(state.pool?.treasures).map(treasureDto),
            compensationCount: str(state.plunder?.plunder_compensation_count),
            battleCount: num(battle.battle_count),
            ...
            solarTerms: solar
                ? {
                    ...solar,
                    terms: list(solar.terms).filter(term => (
                        Number(term.endTime) >= num(group.pet.head.start_time)
                        && Number(term.startTime) <= num(group.pet.head.end_time)
                    )),
                }
                : null,
            plants: [20516, 29004, 25995, 21625, 20154, 21072]
                .map(id => itemDto({ id, count: bag?.get(String(id)) || '0' })),
        };
    }

    /** 拾物小铺目录与背包、节令任一失败都不阻断主状态，降级为 warnings */
    async function readSnapshot() {
        const group = await readGroup();
        const warnings = [];
        try {
            const shop = await operate(SHOP_ID, 7);
            if (!shop.data?.shop) fail('拾物小铺目录缺失');
    ...
    /** 并发读合并为同一个 Promise，避免页面轮询与自动任务重复拉取 */
    ...
    async function getPetDiaryRecords(kind) {
        ...
        return list(reply[selector].logs).map(entry => (kind === 'interact'
            ? {
                time: num(entry.ts) * 1000,
                type: num(entry.type),
                costs: items(entry.costs),
                rewards: items(entry.rewards),
                dogId: str(entry.dog_id),
                skins: list(entry.dog_skin_ids).map(str),
            }
            : {
                time: num(entry.ts) * 1000,
                attackerGid: str(entry.attacker_gid),
                ...
                level: num(entry.attacker_level),
                attackerCharms: list(entry.attacker_charm).map(Number),
                defenderCharms: list(entry.defender_charm).map(Number),
                lost: items(entry.lost_items),
                injected: items(entry.injected_items),
    ...
    async function getPetDiaryFriend(gidInput) {
        ...
        return {
            gid,
            treasures: list(result.info?.treasures).map(treasureDto),
            charms: list(result.info?.defender_charm_ids),
        };
    }
    ...
    async function operatePetDiary(actionInput, input = {}, options = {}) {
        ...
        return serializeMutation(async () => {
            ...
            if (action === 'feed' || action === 'draw') {
                if (action === 'feed' && (num(nurture.stage) !== 1 || num(state.feed?.feed_count) >= base.daily_feed_limit)) {
                    fail('当前不可投喂');
                }
                if (action === 'draw' && (num(nurture.stage) !== 2 || num(state.hunt?.treasure_count) >= base.daily_treasure_limit)) {
                    fail('当前不可寻宝');
                }
                ...
                fail('已领养比熊，请刷新状态');
            } else if (action === 'claimDog' && (num(nurture.stage) !== 2 || nurture.dog_granted)) {
                fail('比熊尚未成年或已经领取');
            } else if (action === 'story') {
                const order = positiveDecimal(input?.order, 'INVALID_STORY', '手记编号');
                if (!list(state.story?.stories).some(s => str(s.order) === order && s.unlocked && !s.claimed)) {
                    fail('手记尚未解锁或已领取');
                }
                params = { order: Number(order) };
            } else if (action === 'seeds') {
                if (!isActive(group.seeds?.head)
                    || !list(group.seeds?.mega_event?.rewards).some(r => r.claimable && !r.claimed)) {
                    fail('当前没有可领取的种子礼包');
                }
                id = SEEDS_ID;
                ...
                // 付费与计数是面板前置条件，官方请求体本身为空。
                // 过期的免费点击不得变成付费刷新，重复的付费点击也不得扣两次。
                if (input?.allowDiamonds || (input?.payment && !['free', 'tickets'].includes(input.payment))) {
                    fail('锦囊刷新不支持使用钻石');
                }
                if (free) {
                    ...
                    if (!Number.isSafeInteger(input?.expectedPaidRefreshCount)
                        || input.expectedPaidRefreshCount !== paidCount) {
                        fail('刷新次数已变化，请刷新状态后重试');
                    }
                    // 线上命令 41 扣 1002 x30。发送前重读点券；
                    // 官方客户端在点券不足时会回退到钻石，本项目一律拒绝。
                    if (!costsAvailable([charmRefreshCost()], await balances())) {
                        fail('点券不足，已停止刷新，不使用钻石');
                    }
                }
                ...
                if (num(nurture.stage) !== 2 || battle.charm_pick_used || !choices.includes(charmId)) {
                    fail('该锦囊不可选择或本轮已经选择');
                }
                params = { charm_ids: [charmId] };
            } else if (action === 'openTreasure') {
                if (!list(state.pool?.treasures).some(t => num(t.status) === 3
                    || (num(t.status) === 2 && num(t.end_at) > 0 && num(t.end_at) <= getServerTimeSec()))) {
                    fail('还没有完成护送的宝藏');
                }
            } else if (action === 'compensation' && num(state.plunder?.plunder_compensation_count) <= 0) {
                fail('当前没有可领取的夺宝补偿');
                ...
                if (num(goods.diamond_cost_count) > 0 || list(goods.cost).some(c => str(c.id) === DIAMOND_ID)) {
                    fail('该商品可能消耗钻石，已阻止兑换');
                }
                if (BigInt(str(goods.purchase_limit)) > 0n
                    && BigInt(str(goods.purchased_count)) + BigInt(count) > BigInt(str(goods.purchase_limit))) {
                    fail('兑换数量超过剩余限购次数');
                }
                if (!costsAvailable(list(goods.cost), await balances(), count)) fail('兑换余额不足');
                params = { goods_id: goodsId, count };
            } else if (action === 'battle') {
                if (!state.hunt?.can_play_plunder || num(battle.battle_count) >= fight.daily_battle_limit) {
                    fail('当前不可夺宝');
                }
                ...
                if (!orders.length || orders.some(order => !list(state.story?.stories)
                    .some(s => num(s.order) === order && s.unlocked))) {
                    fail('手记编号无效');
                }
                params = { orders };
            }
            ...
            let snapshot = null;
            let refreshError = '';
            try { snapshot = await readSnapshot(); } catch (error) {
                refreshError = `操作已成功，刷新失败：${error.message}`;
            }
            return {
                action,
                rewards,
                costs: items(result?.costs),
                result: plain('PetDiaryOperateReply', reply)[selector],
                snapshot,
                refreshError,
    ...
    /** 节令领取走 SolarTermsService，不属于活动 Operate 命令族 */
    async function claimPetDiarySolarTerm(termIdInput) {
        const termId = positiveDecimal(termIdInput, 'INVALID_SOLAR_TERM', '节令编号');
        return serializeMutation(async () => {
            const group = await readGroup();
            if (!isActive(group.pet.head)) fail('萌宠成长日记当前不在活动时间内');
            const solar = await getCurrentSolarTerms();
            const term = list(solar.terms).find(t => String(t.id) === termId
                && Number(t.endTime) >= num(group.pet.head.start_time)
                && Number(t.startTime) <= num(group.pet.head.end_time));
            if (!term?.canClaim) fail('该节令当前不可领取');
            const claimed = await claimSolarTerm(termId);
            let snapshot = null;
            let refreshError = '';
            try { snapshot = await readSnapshot(); } catch (error) {
                refreshError = `领取已成功，刷新失败：${error.message}`;
            }
            return {
                action: 'solar',
                rewards: items(claimed?.rewards),
                snapshot,
                refreshError,
                message: '节令好礼领取成功',
            };
...
module.exports = { createPetDiaryService, OPERATIONS, GROUP_ID, PET_ID, SEEDS_ID, SHOP_ID };
